...
echo "Enter target disk (e.g., /dev/nvme0n1):"
read DISK
echo "Enter hostname:"
read HOSTNAME
echo "Enter username:"
...
echo "$LUKS_PASSWORD" | cryptsetup luksFormat "$ROOT_PART" --type luks2 --batch-mode
echo "$LUKS_PASSWORD" | cryptsetup open "$ROOT_PART" "$LUKS_NAME" --key-file=-
